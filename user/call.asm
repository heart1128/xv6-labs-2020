...
int g(int x) {
   0:	1141                	addi	sp,sp,-16 # 分配16字节的空间
   2:	e422                	sd	s0,8(sp) # sd写指令，s0是栈顶指针，将s0写入到sp+8的位置（保存s0）
   4:	0800                	addi	s0,sp,16 # s0 = sp + 16 (将s0设置为新的栈指针)
  return x+3;
}
   6:	250d                	addiw	a0,a0,3 # a0保存返回值，也保存参数x a0 = a0 + 3 等价a0 = x + 3
   8:	6422                	ld	s0,8(sp)    # ld将double的s0写入sp + 8
   a:	0141                	addi	sp,sp,16 # 恢复栈顶指针
   c:	8082                	ret

000000000000000e <f>:      # f和g内联

int f(int x) {
   e:	1141                	addi	sp,sp,-16 # 分配16字节的栈空间
  10:	e422                	sd	s0,8(sp)        # 保存s0
  12:	0800                	addi	s0,sp,16    # s0 = sp+16,s0作为栈指针
  return g(x);
}
  14:	250d                	addiw	a0,a0,3  # 退栈和g()一样
  16:	6422                	ld	s0,8(sp)
...
void main(void) {
  1c:	1141                	addi	sp,sp,-16  # addi立即数加法指令，sp = sp + (-16)也就是sp栈指针向下移动16字节
  1e:	e406                	sd	ra,8(sp)    # ra存的是函数的返回地址，将函数返回地址放在sp+8的位置
  20:	e022                	sd	s0,0(sp)    # s0作为sp临时栈指针 fp才是栈顶指针
  22:	0800                	addi	s0,sp,16    # s0 = sp + 16（分配栈）
  printf("%d %d\n", f(8)+1, 13);
  24:	4635                	li	a2,13       # li是加载立即数int型, a2 = 13
  26:	45b1                	li	a1,12       # a1 = 12
  28:	00000517          	auipc	a0,0xz0      # pc+0 加载到a0   a0 = (0x0<<12) + pc  （pc=0x28）
  2c:	7a850513          	addi	a0,a0,1960 # 7d0 <malloc+0xea>
  30:	00000097          	auipc	ra,0x0      # 将pc+(0<<12) 加载到ra  ra = pc = 0x30
  34:	5f8080e7          	jalr	1528(ra) #无条件跳转 628 <printf> 将pc设置为ra+1536,并将pc+4写入ra(即进行函数跳转)
  exit(0);
  38:	4501                	li	a0,0    # 结束main。a0置0
